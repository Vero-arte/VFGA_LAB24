...
genes <- as.matrix(
  read.csv( "C:\\Users\\Dell\\Downloads\\heatmap_data.csv",
            sep = "," ,
            header = T,
            row.names = 1))

annotation_col <- read.csv( "C:\\Users\\Dell\\Downloads\\annotation_col.csv", header = T,
                            row.names = 1)

annotation_row <- read.csv( "C:\\Users\\Dell\\Downloads\\annotation_row.csv", header = T,
                            row.names = 1)
...
#take a smaller subset #tomar pequeña muestra
...
# viridis, magma, plasma, cividis, inferno PALETAS PREESTABLECIDAS DE VIRIDIS
